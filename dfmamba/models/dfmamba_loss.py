"""
消融实验版本：Unet+Rest+ConvNext+Loss
保留：对比学习Loss
移除：Fuser (DF_Module) 和 Attention (CoordAtt)
使用简单的特征融合方式替代DF_Module
"""
import torch
...
class SimpleFusionModule(nn.Module):
    """简单特征融合模块：移除DF_Module，使用简单的1x1卷积融合"""
...
class TCMDFMConvNeXtMamba(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training and self.contrast_enable:
            ...
            for i in range(self.n_stages):
                zc_map = self.proj_cnn[i](cnn_features[i])
                ...
                if self.contrast_pixel and self.contrast_pixel_num > 0:
                    ...
                    def gather_positions(feat_map: torch.Tensor) -> torch.Tensor:
                        fm = feat_map.view(B, D, -1)
                        gather_idx = idx.unsqueeze(1).expand(B, D, K)
                        sel = torch.gather(fm, dim=2, index=gather_idx)
                        sel = sel.permute(0, 2, 1).contiguous().view(B * K, D)
...
if __name__ == "__main__":
    ...
    print("消融实验：Unet+Rest+ConvNext+Loss 创建成功!")
