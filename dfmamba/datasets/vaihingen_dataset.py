...
class VaihingenDataset(Dataset):
    ...
    def get_sample_weights(self, minority_classes=[4, 5]):
        # 4: Car, 5: Clutter. Boost sampling for these classes.
        weights = []
        print(f"[Dataset] Scanning {len(self.img_ids)} masks to compute sample weights for classes {minority_classes}...")
        for i in range(len(self.img_ids)):
             # Optim: Read low-res or just check file if possible? No, read full.
             _, mask = self.load_img_and_mask(i)
             mask = np.array(mask)
             
             w = 1.0
             for c in minority_classes:
                 # If class occupies > 0.5% of image, boost significantly
                 ratio = np.sum(mask == c) / mask.size
                 if ratio > 0.005:
                     w += 5.0 * ratio * 100 # Dynamic boost based on amount
                 elif ratio > 0:
                     w += 2.0
        ...
        print(f"[Dataset] Sample weights computed. Max: {weights.max():.4f}, Min: {weights.min():.4f}")
