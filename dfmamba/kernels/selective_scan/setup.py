...
# Custom BuildExtension that bypasses CUDA version check
class CustomBuildExtension(BuildExtension):
    def build_extensions(self):
        # Temporarily disable CUDA version check
        ...
        try:
            ...
            # Replace the check function temporarily if it exists
            if original_check is not None:
                cpp_ext._check_cuda_version = noop_check
            try:
                super().build_extensions()
            finally:
                # Restore original check if we replaced it
                if original_check is not None:
                    cpp_ext._check_cuda_version = original_check
        except RuntimeError as e:
            # If the error is about CUDA version mismatch, try to bypass it
            if "CUDA version" in str(e) and "mismatch" in str(e):
                # Set environment variable to bypass check and retry
                import os
                os.environ['TORCH_CUDA_ARCH_LIST'] = '7.0;8.0;9.0'
                # Try monkey-patching at a lower level
                if hasattr(cpp_ext, '_check_cuda_version'):
...
setup(
    ...
    cmdclass={"bdist_wheel": _bdist_wheel, "build_ext": CustomBuildExtension} if ext_modules else {"bdist_wheel": _bdist_wheel,},
    python_requires=">=3.7",
    install_requires=[
        "torch",
        "packaging",
